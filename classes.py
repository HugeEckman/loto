...
class Game():
    ...
    def start(self):
        ...
        while True:
            ...
            if player1.am_i_win or player2.am_i_loose:
                print(f'Игрок {player1.name} победил!')
                break

            if player2.am_i_win or player1.am_i_loose:
...
class Card():
            ...
            for j in range(1, 10):
                if nums_ptr > 4 and indx_ptr > 4:
                   str.append(' ' * 3) 
                    ...
                    if  val == j:
                        str.append(f'{nums[i][nums_ptr]:>3}')
                        nums_ptr +=1
                        indx_ptr +=1
                    else:
                        str.append(' ' * 3)
    ...
    def cross_number_in_card(self, number: int) -> None:
        ...
        # self._nums_as_string_array[position[0]][position[1]] = ' - '
...
# for _ in range(21):
#     print(f'{bag.number} {len(bag._numbers)} {bag.remains}')